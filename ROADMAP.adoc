...
ReScript                      Rust
─────────────────────────────────────────────
Command.defineCommand()  →   #[tauri::command]
  ~name="my_command"     →   fn my_command()
  ~encode=(args)         →   (args: MyArgs)
  ~decode=(json)         ←   -> Result<T, E>
----

**Contract Checklist:**

* [ ] Document JSON serialization format for common types
* [ ] Create codegen tool for Rust → ReScript command definitions
...
* [ ] Support binary data (Uint8Array → Vec<u8>)
